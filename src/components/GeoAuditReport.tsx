...
const tierColors: Record<LocationScore["tier"], string> = {
  "Top Pick": "bg-success/15 text-emerald-800 border-emerald-300",
  Strong: "bg-blue-100 text-primary-dark border-blue-300",
  Viable: "bg-amber-100 text-warning border-amber-300",
  Stretch: "bg-background-secondary text-foreground-secondary border-border",
};
  ...
  return (
    <div
      className={`rounded-xl border p-5 transition-shadow hover:shadow-lg ${
        isTop
          ? "border-emerald-300 bg-success/10 shadow-sm"
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-foreground">{loc.city}</h3>
            {isTop && (
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-foreground-secondary">
            Est. Roles
          </dt>
        ...
        <div>
          ...
          <dd className="mt-0.5 font-semibold text-foreground text-xs leading-snug">
            {loc.salaryBand}
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-0.5 font-semibold text-xs">
            {loc.remoteEligible ? (
              ...
              <span className="text-foreground-tertiary">On-site</span>
            )}
          </dd>
        </div>
      </dl>
      ...
      {/* Market note */}
      <p className="mt-3 text-xs leading-relaxed text-foreground-secondary italic">
        {loc.marketNote}
      </p>
...
export default function GeoAuditReport({ result }: GeoAuditReportProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-success/20 bg-gradient-to-br from-emerald-50 to-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {mode !== "ai" && (
              <span className="mt-2 inline-block rounded-full bg-background-secondary px-2 py-0.5 text-xs text-foreground-secondary">
                Local scoring
              </span>
            )}
          </div>
        ...
        {/* AI narrative */}
        {aiNarrative && (
          <blockquote className="mt-4 border-l-4 border-emerald-400 pl-4 text-sm italic text-foreground-secondary leading-relaxed">
            {aiNarrative}
          </blockquote>
        )}
      </div>
      ...
      {/* Location grid */}
      <div>
        <h3 className="text-sm font-semibold uppercase tracking-wide text-foreground-secondary mb-4">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-foreground-secondary">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full bg-success" /> 70–100: High demand
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full bg-warning" /> 50–69: Moderate
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full bg-danger" /> &lt;50: Low / Stretch
        </span>
